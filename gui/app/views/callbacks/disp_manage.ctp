 ...
 ***************************************************************************/
   ...
   echo $form->input('batch_id',array('type'=>'hidden','value'=>$batch));
    ...
    if ($callbacks ){
        ...
        $stop =  $form->submit(__('Start',true),  array('name' =>'data[Callback][action]', 'class' => 'button'));
        $start = $form->submit( __('Stop',true), array('name'  =>'data[Callback][action]', 'class' => 'button'));
 
           echo "<table width='35%' cellspacing  = '0' class = 'stand-alone'>";
           $row[] = array(__('Batch',true), $callbacks[0]['Callback']['title']);
