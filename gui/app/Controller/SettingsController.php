...
class SettingsController extends AppController {
	...
	function index() {
	      ...
	      //Fetch form data and process
	         ...
	         if(array_key_exists('ip_radio', $this->request->data['Setting'])){
		     $ip_radio = $this->request->data['Setting']['ip_radio'];
		     unset($this->request->data['Setting']['ip_radio']);
		 }

                 $i=false;	


                foreach ($this->request->data['Setting'] as $id => $entry){
		 ...
		 $data= array('Settings' => $data);
		   ...
		   $this->Session->setFlash(__('Invalid IP address',true)."(".$ip_addr.")".__('Please try again.',true),'error');
		   }
