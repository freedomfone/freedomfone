...
echo $this->Form->create('Cdr',array('type' => 'post','action'=> 'overview'));
...
echo $this->Form->end();
         ...
         if ($message['Message']['new']){ 
	        ...
	        $listen   = $this->element('player',array('url'=>$message['Message']['url'],'file'=>$message['Message']['file'],'title'=>__('New message',true),'id'=>$message['Message']['id']));
                 ...
                 $rows[] = array(
                       ...
                       $this->Time->format('Y/m/d H:i',$message['Message']['created']), 
                       $this->formatting->epochToWords($message['Message']['length']),
    ...
    ///*** HERE STARTS HTML CODE ***///
     ...
     echo $this->Html->tableHeaders(array (__('Application',true),__('No of entries',true),__('Percentage',true)));
     echo $this->Html->tableCells($stat);
     echo $this->Html->tableHeaders(array(false,$total,100*($lamCount+$ivrCount+$pollCount+$otherCount)/$all.' %'),false,array('align' => 'center'));
     ...
     echo $this->Html->div('instruction',__('Total number of messages',true).": ".$message_total);
     if ($message_new){
        if($message_new >=5) { 
             echo $this->Html->div('instruction', __('The table below shows the five most recent new messages',true));
        }
        echo "<table width='600px' cellspacing=0>";
        echo $this->Html->tableHeaders(array (__('Sender',true),__('Time',true),__('Length',true),__('Actions',true),__('Listen',true)));
    ...
    if ($polls){
     ...
     foreach ($polls as $key => $poll){
	   ...
	   $code     = $poll['Poll']['code'];
	   $start    = array($this->Time->format('Y/m/d H:i', $poll['Poll']['start_time']), array('align' => 'center'));
	   $end      = array($this->Time->format('Y/m/d H:i', $poll['Poll']['end_time']), array('align' => 'center'));
     ...
     echo $this->Html->tableHeaders(array(__("Status",true),__("Question",true),__("Code",true),__("Valid votes",true),__("Open",true),__("Close",true),__('Actions',true)), false,array('align' => 'center'));
