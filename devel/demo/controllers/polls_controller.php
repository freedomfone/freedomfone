  ...
  function unlink ($id,$poll_id){
    ...
    function delete ($id){
      ...
      $this->pageTitle = 'Delete poll';
	   ...
	   if($id!=1){
	     ...
	     $this->Session->setFlash('This is a protected demo poll which cannot be deleted.','default',array('class'=>'warning'));
	   }
	     $this->redirect(array('action' => '/index'));
   ...
   $this->pageTitle = 'Edit poll: '.$this->Poll->getTitle($id);   
	...
	if ($this->Poll->saveAll($this->data, array('validate' => 'only'))) {
	 ...
	 $this->Session->setFlash(__("This is a protected demo poll so no edits to it are permitted.",true),'default',array('class' =>'warning'));
