   ...
   if(!$export){
	...
	if( ! $app = $this->data['Cdr']['application']){
       ...
       foreach($ivr as $key => $entry){
	    ...
	    $menu_lam = $form->input('title_lam',array('type'=>'select','options' =>$lam, 'selected' => $session->read('title'),'label'=>''));
	    $menu_ivr = $form->input('title_ivr',array('type'=>'select','options' =>$ivr, 'selected' => $session->read('title'), 'label'=>''));
