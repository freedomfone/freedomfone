...
//debug($line_of_text);
		if($lineCount>0 && $line_of_text[0]!=null){
...
function getBrokenReport($dateArray, $type, $titleCompare='', $cumulative){
	...
	foreach ($dateArray as $call) {  
		...
		if($titleCompare==''||$title==$titleCompare){
			$callDate = $call[0];
			$month = date('M-Y',strtotime($callDate));
			$callerTotal++;
			if($cumulative=='true'){
				//Cumulative Data
				$brokenArray[$month] = $callerTotal;
			}else{
				//Monthly Data
				$brokenArray[$month] ++;
			}
...
function sortArrayByTitle($array, $cumulative){
	$type = 'monthOfYear';
	$titleArray = getTitles($array);
	foreach ($titleArray as $title) {  
		$reportByTitle[$title] = getBrokenReport($array, $type, $title, $cumulative);
	}
	return $reportByTitle;
}
function sortArrayByUnique($array, $cumulative){
	$type = 'monthOfYear';
	$titleArray = getTitles($array);
	$reportByUnique['Uniques'] = getUniqueReport($array, $type, $cumulative);
	$reportByUnique['Total'] = getTotalsReport($array, $type, $cumulative);
...
function saveCSV($report, $seriesKeyArray, $reportTitle){
	//Store to csv
	$csv = array();
	$csvFileName = '/opt/freedomfone/gui/app/tmp/'.$reportTitle.'-'.date('Ymd-his').'.csv';
...
//Break report down into groupings of Months/Day of Months/Dates
function getUniqueReport($dateArray, $type, $cumulative){
	$dateFrom =  strtotime($_SESSION['fromDate']);
	$dateTo = strtotime($_SESSION['toDate']);
	$brokenArray = array();
	while ($dateFrom<=$dateTo) {
...
//Break report down into groupings of Months/Day of Months/Dates
function getTotalsReport($dateArray, $type, $cumulative){
	$dateFrom =  strtotime($_SESSION['fromDate']);
	$dateTo = strtotime($_SESSION['toDate']);
	$brokenArray = array();
