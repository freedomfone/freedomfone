...
cd $SVNROOT; wget http://latest.freeswitch.org/freeswitch-1.0.5-latest.tar.gz
...
cd $SVNROOT; tar zxvf freeswitch-1.0.5-latest.tar.gz
stop

step "FS: First Freeswitch compilation (this can take 1h)"
cd $SVNROOT/freeswitch-1.0.5; sh bootstrap.sh 
cd $SVNROOT/freeswitch-1.0.5; ./configure
cd $SVNROOT/freeswitch-1.0.5; make			
