<?php 
/****************************************************************************
 * edit.ctp	- Edits caller data in phone book
 * version 	- 3.0.1500
 * 
 * Version: MPL 1.1
 *
...
echo $this->Html->addCrumb(__('Edit',true), '/callers/edit/'.$this->request->data['Caller']['id']);

echo $this->Form->create('Caller',array('type' => 'post','controller' => 'callers', 'action'=> 'edit'));
...
echo $this->Form->end();
 ...
 if($data = $this->data){
     ...
     echo $this->Form->create('Caller',array('type' => 'post','action'=> 'edit'));
    ...
    //** START LEFT FRAME **//
     ...
     foreach ($phonenumbers as $key => $number){
             ...
             $delete = $this->Js->link($this->Html->image("icons/delete.png"),
                                   array('controller' => "phone_numbers", "action" => "delete", $number['PhoneNumber']['id'], $data['Caller']['id']),
     ...
     echo $this->Html->tableCells($row, array('class' => 'blue'),array('class' => 'blue'));
     echo "</table>";
     echo $this->Form->input('PhoneNumber.caller_id', array('type' =>'hidden', 'value' => $data['Caller']['id']));
     ...
     //** AJAX: Add phone number **//
       ...
       $add[] = array(
	      ...
	      $this->Js->submit(__('Add phone number',true), array('url' => '/phone_numbers/add', 'update' => '#numbers')),
     	      $this->Form->input('PhoneNumber.caller_id', array('type' =>'hidden', 'value' => $data['Caller']['id'])),
	      );
