...
class NodesController extends AppController{
      ...
      function index(){
      	     ...
      	     $this->pageTitle = 'Voice menus : Menu option files';           
   ...
   function add(){
      	     ...
      	     $this->pageTitle = 'Voice menus : Menu option files : Add';           
	...
	if (!empty($this->data)) {
	   ...
	   if ($title) { 
		...
		if(array_key_exists('urls', $fileOK)){
		      ...
		      $this->Session->setFlash(__('The voice menu option file has been created.', true));
	...
	if(in_array($id,$protect)){ 
		$this->Session->setFlash('The selected menu option file could not be deleted as it is present in the Demo Voice Menu.','default',array('class'=>'warning'));
		$this->redirect(array('action' => '/index'));
		}
	else {
		...
		$this->data = $this->Node->findById($id);
			...
			$this->Session->setFlash('The selected menu option file has been deleted.','default',array('class'=>'message_success'));
		...
		$this->Session->setFlash('The selected menu option file could not be deleted as it is present in one or more Voice menus.','default',array('class'=>'message_success'));
		$this->redirect(array('action' => '/index'));
	     }
	     }
   ...
   function edit($id=null){
      	...
      	$this->pageTitle = 'Voice menus : Menu options files : Edit';           
   	$iid = IID;
   	$ivr_settings = Configure::read('IVR_SETTINGS');
	$path = $ivr_settings['path'].$iid."/".$ivr_settings['dir_node'];
	...
	$protect = array(5,6,7,8);
	if(in_array($id,$protect)){ 
		$this->Session->setFlash('The selected menu option file could not be edited as it is present in the Demo Voice Menu.','default',array('class'=>'warning'));
		$this->redirect(array('action' => '/index'));
		}
	else {
