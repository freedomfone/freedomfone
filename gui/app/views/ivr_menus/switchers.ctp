...
echo "<h1>".__('Languge selectors',true)."</h1>";
	...
	foreach ($switchers as $key => $switcher){
		...
		$delete   = $html->link($html->image("icons/delete.png", array("title" => __("Delete",true))),"/ivr_menus/delete/{$switcher['IvrMenu']['id']}/switcher",null, __("Are you sure you want to delete this language switcher?",true),false);
